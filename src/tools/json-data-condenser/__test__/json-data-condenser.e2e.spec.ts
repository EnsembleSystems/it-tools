...
test.describe('Tool - Json data condenser', () => {
  ...
  test('Condenses valid JSON input correctly', async ({ page }) => {
    ...
    // Confirm two unique object shapes in `users`: one without `email`, one with it
    expect(outputText).toContain('"id": 1');
    expect(outputText).toContain('"email": "david@example.com"');

    // Check repeated shapes are gone
    expect((outputText.match(/"id": \d+/g) || []).length).toBe(2);
  });
